...
{
    public abstract class ManagerTestBase : AbpIntegratedTestBase<ManagerTestModule>
    {
        ...
        protected ManagerTestBase()
        {
            ...
            UsingDbContext(context =>
            {
                new TenantRoleAndUserBuilder(context, 1, "email@prueba.com", StaticRoleNames.Tenants.Admin).Create();
            });
        ...
        protected void UsingDbContext(Action<ManagerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ManagerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ManagerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ManagerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ManagerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ManagerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ManagerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ManagerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ManagerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ManagerDbContext, Task<T>> func)
        {
            T result;
